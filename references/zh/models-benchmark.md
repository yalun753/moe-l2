...
- **首次读取 vs 重复读取**（45K 文档实测）：首次要付冷 prefill（≈550 tok/s，整轮 80-200 秒）；同一会话内追问复用 prompt cache（85-96 秒，prefill 近乎免费）。长文档连续追问请留在同一会话，别每题开新会话。
- 45K 定点检索 64K→384K 全程 ✅ 无衰减；长文**总结**在 `temp ≥ 1.0` 下偶发空正文（**UD 与非 UD 同病**，详见下条；与 ctx 无关）。
- **UD 文件 L0 缺陷（已知事项，2026-09-12 口径修正）**：`blk.0` 专家权重有 ~14% 零字节（Q4_K_M 档，UD 与非 UD **两个不同工具链都是 13.8%/15.5%**），IQ2 档 32-36%，其余层仅 1-2% → 这是**量化位宽相关的产物**，不是"UD 转换写漏数据"（原定性已修正）。行为面：`temperature ≥ 1.0` 时 14.5K 长文总结概率性空正文（同机同条件 6 轮：UD-Q4 3/6、非-UD Q4 2/6，n=6 不可区分）；定点检索不受影响（temp 0.7 下 64K-384K 全过）。**可操作建议只有一个：长输入把温度控制在 0.7-0.8**（换文件无效，已实测）。
- 同卡 **UD-Q4_K_M**（22.1GB）上限 **128K**（13-16 t/s、7963 MiB）：非专家层多占 ~1.9GB，KV 预算被压缩。
- **档位速查**：12G + IQ2 → 256K 甜点（25 t/s、8700 MiB）/ 384K 极限（11358 MiB）；12G + Q4 → 128K；8G 卡（LRU 形态）实测 ~11-17 t/s、命中率 65-76%，建议从小 ctx 起步。
