...
- 45K 定点检索 64K→384K 全程 ✅ 无衰减；长文**总结**在 UD 量化文件上偶发退化是文件侧已知弱区（与 ctx 无关，详见下条）。
- **UD 文件 L0 缺陷（已知事项）**：UD（Unsloth Dynamic）量化的 L0 专家权重有损坏——IQ2_M 的 `blk.0.ffn_gate_exps` 32.2% 零字节、`ffn_up_exps` 36.4%，Q4_K_M 13.8%，其余层仅 1-2%。`temperature ≥ 1.0` 时长文**总结**概率性退化（45K 文档 6 轮坏率：Q4 4/6、IQ2 2/6）；定点检索不受影响（temp 0.7 下 64K-384K 全过）。对策：长输入把温度控制在 0.7-0.8；需要高温长文生成时换非 UD 量化文件（待验证）。
